...
SetVar extractNodes(Space *home, GView g)
{
	SetVar s;
	nodes(home, g, s);
	return s;
...
/**
 * 
 * \brief Subgraph monomorphism problem
 * \ingroup MatchingExamples
 * 
 *  Treats the subgraph monomorphism problem between two ground graphs
 *  and finds the corresponding function.
 */
//@{
class SubgraphMonomorphism : public Example {
   private:
   ...
   public:
      SubgraphMonomorphism(const Options& opt, const string pattern, const string target) 
	 : P(this,loadAmalfi(pattern)), T(this, loadAmalfi(target)), t(this, loadAmalfi(target)), 
	 M(this, P.nodes, T.nodes)// extractNodes(this,P), extractNodes(this, T)) 
	 {
	    ...
	    //monomorphism constraint performing propagation
	    monoInduced(this, P, T, M);
	    branch(this, M, BVAR_SIZE_MIN, BVAL_MIN);  
	 }

      /// Constructor for cloning \a s
      SubgraphMonomorphism(bool share, SubgraphMonomorphism& s) : Example(share,s)
      {
	 ...
	 copy(bool share) {
	    ...
	    return new SubgraphMonomorphism(share,*this);
	 }
...
//@}
   ...
   int main(int argc, char** argv) {
	 ...
	 cout << argv[0] << " <Pattern file> <Target file>" << endl;
      Options o("SubgraphMonomorphism");
      o.solutions = 0;
      o.iterations = 200;
      o.size       = 100;
      o.c_d        = 5;
      ...
      string s1(argv[1]);
      string s2(argv[2]);
      SubgraphMonomorphism* s = new SubgraphMonomorphism(o, s1, s2);
      DFS<SubgraphMonomorphism> e(s,o.c_d,o.a_d);
	delete s;
	do {
	  Example* ex = e.next();
	  if (ex == NULL)
